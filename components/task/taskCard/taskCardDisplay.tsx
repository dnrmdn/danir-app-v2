      ...
      <div className="flex items-start justify-between gap-3 px-5 py-4">
        <div className="flex flex-1 flex-col items-start">
          <div className="mb-2 flex items-center gap-2">
            <span className="rounded-full bg-white/70 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.18em] text-slate-600">
            ...
            {Number(task.reward) > 0 && (
              <div className={`flex items-center gap-1 rounded-full px-2.5 py-1 ${member.bgColor}`}>
                ...
                <span className="text-xs font-semibold text-slate-700">{task.reward} pts</span>
              </div>
            )}
          </div>

          <p className={`line-clamp-2 text-left text-base font-black ${task.completed ? "text-slate-500 line-through" : "text-slate-800"}`}>
            {capitalize(task.label)}
          </p>

          <div className="mt-3 flex flex-wrap items-center gap-2">
            {task.date && (
              <div className="inline-flex items-center gap-1 rounded-full bg-white/70 px-2.5 py-1 text-xs text-slate-600">
            ...
            {!task.date && !task.time && (
              <div className="inline-flex items-center gap-1 rounded-full bg-white/70 px-2.5 py-1 text-xs text-slate-500">
                <Sparkles size={13} />
                Flexible task
              </div>
            )}
