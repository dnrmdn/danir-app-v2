...
import { asAccountType } from "../../_lib/utils";
import { formatAmountInput, parseAmountInput } from "../../_utils/formatAmount";
import { AccountDialogText, AccountForm } from "../../_types";
...
export function AccountDialog({ t, open, onOpenChange, loading, form, setForm, onSave }: AccountDialogProps) {
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md rounded-[1.25rem] border-border bg-card p-4 text-foreground dark:border-white/10 dark:bg-[#07111f] dark:text-white sm:rounded-3xl sm:p-6">
        <DialogHeader>
          <DialogTitle className="text-base font-black sm:text-lg">{t.addAccount}</DialogTitle>
          <DialogDescription className="text-xs text-muted-foreground sm:text-sm">{t.accountDescLong}</DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-2">
              ...
              <Input
                inputMode="numeric"
                value={formatAmountInput(form.initialBalance)}
                onChange={(e) =>
                  setForm((prev) => ({
                    ...prev,
                    initialBalance: parseAmountInput(e.target.value),
                  }))
                }
                className="border-border bg-muted/50 text-xs dark:border-white/10 dark:bg-white/5 sm:text-sm"
              />
